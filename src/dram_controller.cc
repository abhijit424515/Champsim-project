...
void MEMORY_CONTROLLER::reset_remain_requests(PACKET_QUEUE *queue, uint32_t channel)
{
  for (uint32_t i = 0; i < queue->SIZE; i++)
  {
    if (queue->entry[i].scheduled)
    {
      ...
      DP(if (warmup_complete[op_cpu]) { cout << queue->NAME << " instr_id: " << queue->entry[i].instr_id << " swrites: " << scheduled_writes[channel] << " sreads: " << scheduled_reads[channel] << endl; });
    }
  }
...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
  ...
  for (uint32_t i = 0; i < queue->SIZE; i++)
  {
    ...
    { // should we check this or not? how do we know if bank is busy or not for all requests in the queue?
